...
Når vi bruker excel (eller lignende programmer) for å mate inn data kan vi bruke datavalidering for å kontrollere at den data vi fører inn er hva vi forventer. Datavalidering innebærer att hver kolonne, eller rad får et sett med regler som inndata må ha. F.eks. når vi taster inn data fra en VO<sub>2maks</sub> test vet vi att data ikke kan være negative, og de overstiger mest sannsynlig ikke 97 ml kg<sup>-1</sup> min<sup>-1</sup>. Vi kan med denne kunnskapen skape en regel som sier at vi tillater desimaltall mellom, la oss si, 25 og 100. 
...
En **reproduserbar dataanalyse** inneholder all informasjon som kreves for å gjenskape analyseresultatene. En **transparent dataanalyse** inneholder også beskrivelser av hvorfor og hvordan man har valgt å lage analysen. For å gjøre dette mulig så kreves ytterligere struktur til et prosjekt. 
