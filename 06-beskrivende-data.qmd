...
# Workshop: Beskrive data og beskrive med data

## Introduksjon

I denne workshopen som omhandler beskrivende statistikk ønsker vi å fokusere på følgende læringsmål:

-	Identifisere hvordan data kan sammenstilles for å besvare problemstillinger
-	Bruk av teknikker for å beskrive data, f.eks. figurer og tabeller med sentralmål og spredning
-	Bruk av Jamovi for å sammenstille og filtrere data samt lage nye variabler.

Når dere identifiserer og konkretiserer problemstillinger fra problembeskrivelsen under kan dere lage en plan for dataanalyse i Jamovi som inneholder:

-	Sammenstille data over flere kategorier
-	Lage figurer og tabeller
-	Lage nye variabler basert på formler
-	Filtrere data

Presentasjonen bør inneholde tanker rundt hvorfor dere har fremstilt data på valgt måte i presentasjon til klubbledelsen. 
...
opponents <- rep(c("Storhamar", "Frisk Asker Hockey", "Oilers", "Valerenga", "Sparta Sarpsborg",
     "Arctic Eagles Narvik", "Lorenskog IK", "Stjernen", "Comet Halden"),3)

# Random set of opponents
opp <- sample(opponents, 27, replace = FALSE)
sesong <- c(rep("pre", 7), rep("mid", 10), rep("end", 10))
...
antall_polser <- data.frame(year = rep(c(2022, 2023, 2024), each = 27), 
                            polser_kjop = rep(c(1000, 1100, 1200), each = 27), 
                            kaffe_kjop = rep(c(800, 800, 800), each = 27))
...
write.csv2(polsestatistikk, "data/polsestatistikk.csv")
...
Lillehammer Ishockeyklubb trenger hjelp. Det viser seg at de må bli bedre på å planlegge innkjøp av pølser og kaffe til hjemmekamper for å tjene mer penger til klubben. Økonomidirektøren har identifisert de lokale pølsene og kaffe som to store muligheter til å tjene mer penger. Han ønsker nå å vite hvordan man bedre kan planlegge innkjøp av pølser og kaffe for å maksimere profitten. Pølsene kjøpes fra en lokal slakter og er ferskvare, hvis pølsene ikke blir solgt i løpet av kampen må de destrueres. Destruksjon av pølser innebærer derfor tap tilsvarende innkjøpsprisen, som er 11.28 NOK. For hver pølse som blir solgt tjener klubben 38.72 NOK (salgspris 50 NOK). Kaffe er ikke ferskvare, men holder bare en sesong. Etter avsluttet år må kaffen destrueres. Lillehammer kjøper kaffe for 2 NOK per kopp og tjener 28 NOK per kopp (salgspris 30 NOK).

[Her finner du et datasett](data/polsestatistikk.csv) med historiske data over hjemmekamper i Kristins hall på Lillehammer. Hallen kan ta 2500 tilskuere og det finnes 1700 sitteplasser. I de historiske dataene finner dere år for sesongen (ar, 2022 = 21/22, 2023 = 22/23 og 2024 = 23/24), hvilket lag man møter (motstand), hvilken del av sesongen kampen ble spilt (pre, før jul; mid, Januar-Februar; end, Februar-Mars), hvor mange tilskuere som betalte inngang (publikum), antall solgte pølser (antall_polser), antall solgte kaffekopper (antall_kaffe),  antall pølser man kjøpte fra slakteren (polser_kjop) og hvor mye kaffe som ble kjøpt inn til hver kamp (antall, kaffe_kjop).

Klubben ønsker ikke å endre prisene på varene, men bare få en oversikt over hvordan man kan planlegge innkjøp bedre. Lag en presentasjon som gir forslag til hvordan LIK kan planlegge innkjøp av kaffe og pølser på en bedre måte. Klubbledelsen forventer at du tar hensyn til de variablene i datasettet som kan være interessante for å ta rett beslutning (f.eks. motstand, år, del av sesong). Lag også en beregning av hvor mye man kan spare ved å følge deres forslag.

I løsningen som dere presenterer til klubbledelsen kan dere bruke følgende case for å beskrive hvor mange pølser og hvor mye kaffe som bør kjøpes inn.

|Motstand |Sesong|Publikum |Antall pølser som forventes selges |Antall kaffe som forventes selges|
|---|---|---|---|---|
|Storhamar|pre|?|?|?|
|Stjernen |pre|?|?|?|
|Vålerenga|pre|?|?|?|
