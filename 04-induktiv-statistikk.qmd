...
::: {.column-margin}
 ...
 &#x1F4F9; Forelesning: [Intro til statistisk inferens](https://inn.cloud.panopto.eu/Panopto/Pages/Viewer.aspx?id=5a86dc40-64fd-43c5-bf9f-b155008ad7f6){target="_blank"}.
...
::: {.column-margin}
 ...
 &#x1F4F9; Forelesning: [P-verdier og hypotesetesting](https://inn.cloud.panopto.eu/Panopto/Pages/Viewer.aspx?id=cfefd71c-06eb-4756-a98b-b1560079ad8c){target="_blank"}.
...
::: {.column-margin}
 ...
 &#x1F4F9; Forelesning: [Statistisk styrke](https://inn.cloud.panopto.eu/Panopto/Pages/Viewer.aspx?id=bbb99af4-6a2e-4db3-81f4-b1560081f65f){target="_blank"}.
...
::: {.column-margin}
 ...
 &#x1F4F9; Forelesning: [Mer om p-verdier](https://inn.cloud.panopto.eu/Panopto/Pages/Viewer.aspx?id=ce4f66d2-08e7-4b16-a16d-b1560085ef63){target="_blank"}.
...
#| fig-height: 3
#| label: fig-pval-null
#| fig-cap: "Fordeling av p-verdier etter 10000 sammenligninger av utvalg fra den samme populasjonen, noe som betyr at nullhypotesen er riktig."
...
#| fig-width: 6
#| fig-height: 4
#| label: fig-pval-confint
#| fig-cap: "Den observerte p-verdien er arean under kurven i nullhypotesen for verdier som er mer ekstreme enn den observerte verdien (i begge retninger fra null). Nullhypotesen konstrueres ved hjelp av *spredningen* i dataene. Konfidensintervallet er 95% av arean under kurven i utvalgsfordelingen som konstrueres med gjennomsnitt og spredning fra den observerte dataen. Når et 95% konfidensintervall ikke overlapper nullhypotesen vil p-verdien være mindre enn 0.05. I figuren er nullhypotesen representert med en lilla kurve og konfidensintervallet med en grønn kurve." 
...
data.frame(x = c(-0.4, 0.5)) %>%
  ...
  annotate("text", x = 0.35, y = 2.5, 
           color = "purple",
           label = paste("p = ", round(2*pt(0.15/sigma_se, df = n-2, lower.tail = FALSE),3)), size = 4) +
...
## P-verdier og overraskelsefaktoren

La oss si at vi gjennomfører et eksperiment med myntkast. Med et balansert mynt vil vi ha like stor sjanse for å få kron som mynt. Vi hadde derfor ikke vært overrasket om vi hadde fått 5 kron og 5 mynt i en serie med 10 myntkast. Men hvis vi får 10 kron i en serie med 10 myntkaster, da hadde vi muligens vært litt mer overrasket. Det viser seg at p-verdien for denne observasjonen (10 kron i en serie med 10 myntkaster) hvis nullhypotesen om en balansert mynt er riktig er `r format(0.5^10, scientific = FALSE)` [@cole2021]. Med noe matematikk viser det seg at vi kan regne oss tilbake til antallet kron vi fikk i serien myntkaster ved å beregne

$$-\operatorname{log}_2(0.0009765625) = 10$$
Det å ta den negative logaritmen med basen to av p-verdien gir oss et tall som vi kan tolke som det antall myntkast i en serie hvor alle myntkaster er enten kron eller mynt. Dette tallet kalles for overraskelseindeksen og benevnes som $s$-verdien [@cole2021]. 

::: {.column-margin}
s-verdien har fått sitt navn efter "surprisal" (som i surprise) eller "Shannon information" (som i Claude Shannon) (se @cole2021 og [Information Content](https://en.wikipedia.org/wiki/Information_content) på Wikipedia).
:::
...
I en studie fant vi nylig at varmetrening ga en 30 gram økning i Hb-masse hos veltrente langrennsløpere [@ronnestad2022], p-verdien for denne observasjonen var 0.009, noe som gir et overaskelseindeks på `r round(-log2(0.009),1)`. Vi kan dermed si at dette resultatet litt mindre overraskende enn å få 7 kron i en serien med 7 myntkaster. Samtidig ga intervensjonen ingen markert effekt på prestasjon (p=0.367, s=`r round(-log2(0.367),1)`), denne observasjonen var mindre overraskende enn å få 2 kron i en serie med 2 myntkaster. 
...
#| fig-height: 3
#| label: fig-pval-sval
#| fig-cap: "Forhold mellom p-verdier og overraskelseindeksen (s-verdien). En <span style='color:green'>p-verdi på 0.05</span> gir en overraskelseindeks på 4.32, mens en <span style='color:orchid'>p-verdi på 0.01</span> gir en overraskelseindeks på 6.64."



data.frame(pval = seq(from = 0, to = 1, by = 0.00001), 
           sval = -log2(seq(from = 0, to = 1, by = 0.00001))) %>%
  ggplot(aes(x = pval, y = sval)) + 
  
  scale_x_continuous(breaks = c(0.01, 0.05, 0.2, 0.5, 0.8, 0.95, 1), 
                     labels = c(0.01, 0.05, 0.2, 0.5, 0.8, 0.95, ""), 
                     limits = c(-0.011,1), 
                     expand = c(0,0), 
                     guide = guide_axis(n.dodge=2)) +
  
  scale_y_continuous(breaks = c(0, 1, -log2(0.05), -log2(0.01), 8, 10, 12, 14),
                     labels =  c(0, 1, round(-log2(0.05),2), round(-log2(0.01),2), 8, 10, 12, 14), 
                     limits = c(0, 15), 
                     expand = c(0, 0)) +
  ...
  annotate("segment", 
           x = c(0.01, 0.05, 0.01, 0.05),
           xend = c(0.01, 0.05, -0.01, -0.01), 
           y = c(0,0, -log2(0.01), -log2(0.05)), 
           yend = c(-log2(0.01), -log2(0.05), -log2(0.01), -log2(0.05)), 
           color = c("orchid", "green", "orchid", "green"), 
           linewidth = 1.5) +
