...
Vi har nu mulighet for å beregne antallet deltakere som kreves for å oppnå en statistisk styke på 80%. Til denne beregning bruker vi følgende tall:
...
Ditt statistikkprogram (for eksempel Jamovi) kan beregne antallet deltakere som kreves for å oppnå en gitt statistisk styrke
...
#| fig-height: 3
#| label: fig-power
#| fig-cap: " (Figuren er inspirert av " 



## Calculate the standard error for the sampling distribution
...
dt2_limit_lower <- function(x) {
    ...
    y[x < -qt(0.99999, df = n-2) * sigma_se  |  x > -qt(0.975, df = n-2) * sigma_se] <- NA
    return(y)
}

dt2_limit_upper <- function(x) {
    y <- dt2(x, mu = 0, nu = sigma_se, df = n-2, ncp = 0)
    y[x < qt(0.975, df = n-2) * sigma_se  |  x > qt(0.999, df = n-2) * sigma_se] <- NA
    return(y)
}
...
# Parameters for the Null and Alternative Hypotheses
mu0 <- 0       # Mean of the null hypothesis
sigma <- 1     # Population standard deviation of measurements
n <- 63        # Sample size for both groups

# Calculate the standard errors for the sampling distributions
sigma_se <- sigma / sqrt(n)

# Effect size is calculated relative to population standard deviation
effect_size <- 0.5  
mu1 <- mu0 + effect_size * sigma  # Mean of the alternative hypothesis based on effect size

alpha <- 0.05  # Significance level

# Critical value for a one-sided test
z_crit <- qnorm(1 - alpha) * sigma_se + mu0  # Adjust critical value for the null hypothesis SE

# Data generation for the curves
x <- seq(mu0 - 3 * sigma_se, mu1 + 3 * sigma_se, length.out = 1000)
h0 <- dnorm(x, mean = mu0, sd = sigma_se)
h1 <- dnorm(x, mean = mu1, sd = sigma_se)
...
# Plot
ggplot(df, aes(x)) +
  geom_line(aes(y = h0), col = "blue") +
  geom_line(aes(y = h1), col = "red") +
  geom_area(aes(y = h1), fill = "red", alpha = 0.2, xlim = c(z_crit, max(x))) +
  geom_area(aes(y = h0), fill = "blue", alpha = 0.2, xlim = c(min(x), z_crit)) +
  geom_vline(xintercept = z_crit, linetype = "dashed", color = "black") +
  labs(title = "Power Analysis with Effect Size 0.5 (Sampling Distributions)",
       y = "Density", x = "Measurement Units") +
  theme_minimal()
